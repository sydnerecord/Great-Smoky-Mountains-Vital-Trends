...
#update path
github_path = "/Users/jgradym/Documents/GitHub/Great-Smoky-Mountains-Vital-Trends"
...
source(file.path(github_path, "Main/5.GRSM_Fish_3-Pass_Summary.R"))
...
effort_samples_stream <- three_pass %>%
  ...
  distinct(Stream, LOC_NAME, Date, Year) %>%          # one sample per site-date
  count(Year, Stream, name = "n_samples") %>%     # samples per Year × Stream
  ungroup()
effort_samples_stream 
# ===============================================================
